...
for lang in langs:
    kd = Path("reports/keywords") / f"{lang}_keybert.json"
    yd = Path("reports/keywords") / f"{lang}_yake.json"
    kb = load_pairs(kd)
    yk = load_pairs(yd)
    ...
    print(f"{lang} | KeyBERT: {len(kb_terms):>3} terms, avg_len={avg_len_kb:.1f} | "
          f"YAKE: {len(yk_terms):>3} terms, avg_len={avg_len_yk:.1f} | Jaccard={jac:.3f}")